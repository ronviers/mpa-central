"""Two-temperature OU — CONTROL primitive (rung 4: prescribed X).

Exact discrete OU (stationary variance 1, relaxation τ_relax). The
perturbed branch gets a drift b = X·h·(1-a) so the measured response is
    χ(τ) = X·(1 - C(τ)),    C(τ) = a^τ = exp(-τ/τ_relax).
The FDR locus is a straight line of slope X in the (1-C, χ) plane.
X=1 reduces to equilibrium OU; X<1 is a known FDT violation.
"""
...
def _init_state(tau_relax: float, X: float):
    a = math.exp(-1.0 / float(tau_relax))
    def factory(rng: np.random.Generator, n_real: int) -> dict:
        x = rng.standard_normal((n_real, 1))  # stationary N(0,1)
        return {
...
def _advance(state: dict, h_extra_per: float, rng: np.random.Generator):
    ...
    # Drift on the perturbed branch realizing a prescribed FDT-violation X.
    b = state["X"] * h_extra_per * (1.0 - a)
    state["unp_prev"] = state["unp"].copy()
    state["per_prev"] = state["per"].copy()
    state["unp"] = a * state["unp"] + xi
    state["per"] = a * state["per"] + xi + b
...
def multi_window_fdr_iter(tau_relax, X, t_w, t_obs, tau_windows, h_field,
                           n_realizations, seed, n_sample_times,
                           progress_every=0, xdot_kind="velocity") -> Iterator[dict]:
    return stream_fdr(
        substrate="two_temp_ou", op_label=f"X{X:g}",
        op_params={"tau_relax": float(tau_relax), "X": float(X), "T_eff": 1.0 / X},
        ...
        init_state=_init_state(tau_relax, X), advance_one_sweep=_advance,
