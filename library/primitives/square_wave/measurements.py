"""Square wave — CONTROL primitive (the character zero / tare).

x(t) = sign(sin(2π·(t + φ_r)/P)),  φ_r ~ Uniform[0, P) per replica.

Phase-randomized so the ensemble is phase-stationary → a single
character point. Deterministic, driven, NO bath: C(t,t_w) is the
phase-averaged triangular autocorrelation and never relaxes to r.

Perturbed branch is phase-shifted by h_field steps:
    x_per(t) = sign(sin(2π·(t + φ_r + h_field)/P)).
So the perturbed flips land slightly off the unperturbed ones; χ reads
the (driven, non-relaxational) response of a phase-shifted square wave.

The clock t is carried in state and advances every sweep (through phase
A, kernel warmup, and measurement) so the wave oscillates continuously.
"""
...
def _square(t: float, phi: np.ndarray, P: float) -> np.ndarray:
    s = np.sin(2.0 * math.pi * (t + phi) / P)
...
def _advance(state: dict, h_extra_per: float, rng: np.random.Generator):
    ...
    state["unp"] = _square(t, phi, P)
    # Perturbed branch: fixed phase shift of h_extra_per steps (constant
    # through the measurement window, 0 during phase A).
    state["per"] = _square(t + h_extra_per, phi, P)
...
def multi_window_fdr_iter(P, t_w, t_obs, tau_windows, h_field,
    ...
    return stream_fdr(
        substrate="square_wave", op_label=f"P{P:g}",
